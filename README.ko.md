...
**Cursor**는 install symlink가 없다 — 실행할 때마다 팩을 넘긴다:
...
| **Cursor Agent CLI** | `--plugin-dir` 후 슬래시 커맨드 또는 자연어 ([Cursor 설정](docs/cursor.md)) |
...
이후 에이전트를 재시작한다 (Cursor: `--plugin-dir`로 다시 실행). 문제 해결은 [설치](#설치)를 본다.
