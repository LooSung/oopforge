...
>
> 支持 Java (Spring) · Python (FastAPI / Flask)。**3 层 (Controller/Service/Repository)** 或 **hexagonal/clean** 任选，OpenAPI/Swagger 默认开箱即用。
...
### `/oopforge:route` — 不确定从哪里开始时

不强制走完整 workflow，根据用户意图只推荐最小单位的 skill/command。

```text
/oopforge:route 想在现有 payment domain 加一个 refund use case
/oopforge:route 只做一个 Email value object
/oopforge:route 从零开始一个 membership domain
```

### Stack 标识符 (`/oopforge:skeleton`)
...
| `python-flask-layered` | 3 层 (Blueprint/Service/Repository) | Flask 服务 |

所有 backend skeleton 都 **默认启用 OpenAPI/Swagger UI** (springdoc / FastAPI 内置 / flask-smorest)。
...
/oopforge:skeleton java-spring-layered           # 或 java-spring-hexagonal
...
└── lang/
    ├── api/         OpenAPI / Swagger 约定 (springdoc, FastAPI, flask-smorest)
    ├── java/        Spring 3 层 (layered) + Spring hexagonal, JPA repository
    └── python/      FastAPI 3 层 + FastAPI clean + Flask 3 层,
